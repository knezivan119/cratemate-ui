...
const childrenData = ref( { data: [] } )
const childrenError = ref( null )
const junkData     = ref( { data: [] } )
...
vi.mock( 'src/queries/crateQuery', () => ( {
    ...
    useCrateChildren: () => ( { data: childrenData, error: childrenError } ),
    useUpdateCrate:   () => ( { mutateAsync: updateMutate } ),
    useDeleteCrate:   () => ( { mutateAsync: deleteMutate } ),
} ) )

vi.mock( 'src/queries/junkQuery', () => ( {
    useJunkListInCrate: () => ( { data: junkData, isLoading: junkLoading, error: junkError } ),
} ) )
...
describe( 'crateEditUse', () => {
    ...
    it( 'parentRoute is /crates when no parent', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'Root', parent_id: null } }
        await nextTick()
    ...
    it( 'parentRoute points to parent crate page when crate has a parent', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'Sub', parent_id: 'parent-x' } }
        await nextTick()
        expect( harness.result.parentRoute.value ).toBe( '/crates/parent-x' )
    } )

    it( 'loadError surfaces any of the three query errors', async () => {
        crateError.value = { message: 'crate gone' }
        await nextTick()
        expect( harness.result.loadError.value ).toEqual( { message: 'crate gone' } )
    } )
    ...
    it( 'openAddCrate opens the form dialog with parentId of current crate', () => {
        ...
        expect( lastDialogConfig ).toMatchObject( {
            component:      { name: 'CrateFormDialog' },
            componentProps: { parentId: 'crate-1' },
        } )
    } )
    ...
    it( 'openAddJunk opens capture dialog with crate context once loaded', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'Garage' } }
        await nextTick()
        harness.result.openAddJunk()
        expect( lastDialogConfig ).toMatchObject( {
            component:      { name: 'JunkCaptureDialog' },
            componentProps: { crateId: 'crate-1', crateName: 'Garage' },
        } )
    } )

    it( 'openMove opens picker; on OK with a different crate updates parent_id', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'X', parent_id: 'old' } }
        await nextTick()
        updateMutate.mockResolvedValue( {} )

        harness.result.openMove()
        expect( lastDialogConfig.component.name ).toBe( 'CratePickerDialog' )
        await dialogOnOkCallback( { id: 'new-parent' } )

        expect( updateMutate ).toHaveBeenCalledWith( {
            id:      'crate-1',
            payload: { parent_id: 'new-parent' },
        } )
    } )

    it( 'openMove ignores picking the same crate as itself', async () => {
        crateData.value = { data: { id: 'crate-1' } }
        await nextTick()

        harness.result.openMove()
        await dialogOnOkCallback( { id: 'crate-1' } )

        expect( updateMutate ).not.toHaveBeenCalled()
    } )

    it( 'moveToRoot calls updateCrate with null parent_id', async () => {
        crateData.value = { data: { id: 'crate-1', parent_id: 'p' } }
        await nextTick()
        updateMutate.mockResolvedValue( {} )
    ...
    it( 'moveToRoot is a no-op when crate is already root', async () => {
        crateData.value = { data: { id: 'crate-1', parent_id: null } }
        await nextTick()
        await harness.result.moveToRoot()
        expect( updateMutate ).not.toHaveBeenCalled()
    } )
    ...
    it( 'confirmDelete opens dialog; on OK deletes and replaces route', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'Bin', parent_id: null } }
        await nextTick()
    ...
    it( 'confirmDelete after delete navigates to parent if one exists', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'Bin', parent_id: 'parent-x' } }
        await nextTick()
        deleteMutate.mockResolvedValue( {} )
    ...
    it( 'delete failure notifies but does not navigate', async () => {
        crateData.value = { data: { id: 'crate-1', name: 'Bin', parent_id: null } }
        await nextTick()
        deleteMutate.mockRejectedValue( { body: { error: { message: 'denied' } } } )

        harness.result.confirmDelete()
        await dialogOnOkCallback()

        expect( $qNotify ).toHaveBeenCalledWith( expect.objectContaining( {
            color: 'negative', message: 'denied',
        } ) )
